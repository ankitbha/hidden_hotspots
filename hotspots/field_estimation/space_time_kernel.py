...
for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    t = (idx-df.index[0]).total_seconds()

    if(len(val)<30):
        continue
        
    x = np.array([[la,lo,t] for la, lo in zip(lat, long)])
    y = np.array(val)
...
for i in tqdm(range(ws,len(x_tr[ws:-ws]))):
    ...
    k_space = gpflow.kernels.SquaredExponential(active_dims=[0,1])
    k_time = gpflow.kernels.SquaredExponential(active_dims=[2])
    k_spacetime = gpflow.kernels.SquaredExponential(active_dims=[0,1])*k_time
    
    gp = gpflow.models.GPR((x_train.astype(np.float64), y_train),kernel=k_space+k_time+k_spacetime)
    
    pred, _ = gp.predict_f(x_test.astype(np.float64))
